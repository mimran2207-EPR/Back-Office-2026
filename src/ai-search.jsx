...
function AISearch() {
  ...
  const aiAsk = async (text) => {
    ...
    // Try real LLM via window.claude.complete if available, fallback to local insight
    let aiPayload = null;
    let aiText = null;
    try {
...
נתונים חיים:
- פניות פתוחות: ${d.stats?.open?.v}
- SLA כללי: ${d.stats?.sla?.v}%
- פניות דחופות: ${d.stats?.urg?.v}
- זמן טיפול ממוצע: ${d.stats?.avg?.v} ${d.stats?.avg?.unit}
- מחלקות: ${(d.departments||[]).map(x=>`${x.name} (${x.open} פניות, SLA ${x.sla}%)`).join('; ')}
- פניות אחרונות: ${(d.requests||[]).slice(0,5).map(r=>`${r.id} ${r.title} - ${r.status}`).join('; ')}

ענה בעברית, קצר ותכליתי (עד 4 משפטים). אם נשאלת על מספרים - השתמש בנתונים שלמעלה.
...
שאלת המשתמש: ${text}`;
    ...
    if (!aiText) {
      ...
      aiPayload = aiInsight(text);
      aiText = aiPayload.title;
    }
    setConvo(c => [...c, { role:'ai', text: aiText, payload: aiPayload }]);
    setThinking(false);
  };
